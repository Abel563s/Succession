                            ...
                            <div class="flex items-center gap-4">
                                <div class="bg-slate-50 p-2 rounded-2xl border border-slate-100">
                                    @if($criticalRole->signature_path)
                                        <img src="{{ asset('storage/' . $criticalRole->signature_path) }}" class="h-20 object-contain" alt="Signature">
                                    @else
        ...
        @if(auth()->user()->isAdmin())
            <div class="bg-rose-50 rounded-[2rem] border border-rose-100 p-8 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-rose-600/70 text-sm font-medium">Permanently remove this evaluation record from the system.</p>
                </div>
                <form action="{{ route('admin.critical-roles.destroy', $criticalRole) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
