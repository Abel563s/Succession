    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-black text-slate-900 tracking-tight">New Critical Role Evaluation</h1>
                    <p class="text-slate-500 font-medium text-sm">Assess position criticality and succession readiness.</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin.critical-roles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
                ...
                <div class="p-8 space-y-8">
                    @for($i = 1; $i <= 3; $i++)
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-2xl bg-slate-50 border border-slate-100 relative">
                            <div class="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-10 bg-white border border-slate-200 rounded-lg flex items-center justify-center font-black text-[#00ADC5] shadow-sm text-xs">
                                {{ $i }}
                            </div>
                            <div class="space-y-2">
                                ...
                                <input type="text" name="successor_{{ $i }}_name" value="{{ old('successor_'.$i.'_name') }}"
                                       class="w-full px-4 py-3 bg-white border border-slate-200 focus:border-[#00ADC5] focus:ring-4 focus:ring-[#00ADC5]/10 outline-none transition-all"
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="successor_{{ $i }}_readiness"
                                    ...
                                    <option value="Long Term" {{ old('successor_'.$i.'_readiness') == 'Long Term' ? 'selected' : '' }}>Long Term</option>
                                </select>
                            </div>
                        </div>
                    @endfor
                ...
                <div class="p-8 space-y-6">
                    <div class="space-y-2">
                        ...
                        <textarea name="mitigation_plan" required rows="4"
                                  ...
                                  class="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#00ADC5] focus:ring-4 focus:ring-[#00ADC5]/10 outline-none transition-all resize-none">{{ old('mitigation_plan') }}</textarea>
