    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
            <div class="flex flex-col">
                <h1 class="text-2xl font-black text-slate-900 tracking-tight">Leadership Assessments</h1>
                <p class="text-xs text-slate-400 font-medium">Evaluating leadership competencies and performance readiness.</p>
            </div>
            ...
            <div class="hidden lg:flex items-center justify-center">
                <div class="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-xl hover:border-[#111111]/20 transition-all duration-500 group/badge">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest group-hover/badge:text-slate-600 transition-colors">Leadership Assessments</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex md:justify-end">
                <a href="{{ route('admin.leadership.create') }}" 
                   class="bg-gradient-to-r from-[#111111] to-[#00333B] hover:to-[#111111] text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all duration-300 shadow-lg shadow-[#111111]/20 hover:shadow-[#111111]/30 hover:scale-[1.02] active:scale-95 group/btn relative overflow-hidden">
                    ...
                    <span class="relative z-10">New Assessment</span>
                </a>
            </div>
        </div>
        ...
        <div class="flex justify-center">
            <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm inline-block">
                <form action="{{ route('admin.leadership.index') }}" method="GET" class="flex flex-wrap items-end justify-center gap-4">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($records as $record)
                            <tr class="hover:bg-slate-50/50 transition-colors group text-sm">
                                ...
                                <td class="px-6 py-5">
                                    <p class="text-[10px] font-black uppercase text-slate-400 tracking-widest">{{ $record->department }}</p>
                                    <p class="text-xs font-bold text-slate-600 mt-0.5">{{ $record->line_manager }}</p>
                                </td>
                                <td class="px-6 py-5 text-center">
                                    <div class="flex flex-col items-center">
                                        <span class="text-sm font-black text-slate-900">{{ number_format($record->overall_score, 1) }} / 5.0</span>
                                        <div class="w-20 h-1.5 bg-slate-100 rounded-full mt-1 overflow-hidden">
                                            <div class="h-full bg-[#D4AF37]" style="width: {{ ($record->overall_score / 5) * 100 }}%"></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5 text-center">
                                    <span class="inline-flex px-3 py-1 rounded-lg text-[10px] font-black uppercase {{ $record->status == 'published' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100' }}">
                                        {{ $record->status }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        @if(auth()->user()->isAdmin() || (auth()->user()->isManager() && ($record->approval_status === 'Pending' || (($record->created_by ?? $record->user_id) == auth()->id() && (!$record->approval_status || $record->approval_status === 'Pending')))))
                                            <a href="{{ route('admin.leadership.edit', $record) }}" 
                                        ...
                                        @if(auth()->user()->isAdmin())
                                            <form action="{{ route('admin.leadership.destroy', $record) }}" method="POST" 
