        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.leadership.edit', $leadership) }}" class="bg-slate-900 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10 flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <div class="bg-slate-50 p-3 rounded-2xl border border-slate-100 inline-block relative z-10">
                            @if($leadership->signature_path)
                                <img src="{{ asset('storage/' . $leadership->signature_path) }}" class="h-20 object-contain" alt="Signature">
                            @else
        ...
        @if(auth()->user()->isAdmin())
            <div class="bg-rose-50 rounded-[2rem] border border-rose-100 p-8 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-rose-600/70 text-sm font-medium">Permanently remove this assessment from the system.</p>
                </div>
                <form action="{{ route('admin.leadership.destroy', $leadership) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this assessment?');">
                    @csrf
                    @method('DELETE')
