        ...
        <form action="{{ route('admin.sd.update', $sd) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden" x-data="{ 
                ...
                <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
                    ...
                    <button type="button" @click="rows.push({position: '', current_holder: '', candidates: '', timeline_progress: '', competency_progress: '', development_progress: '', kpi_metrics: '', monitoring_progress: '', readiness_rating: 'Ready Now'})" class="text-xs font-black text-[#00ADC5] uppercase tracking-widest hover:text-[#008d9e] flex items-center gap-2 bg-[#f0fbfd] px-4 py-2 rounded-xl transition-all border border-[#00ADC5]/10">
                        ...
                        <tbody>
                            <template x-for="(row, index) in rows" :key="index">
                                <tr class="border-b border-slate-50 hover:bg-slate-50/30 transition-colors group">
                                    ...
                                    <td class="px-4 py-4">
                                        <input type="text" name="position[]" required x-model="row.position" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:border-[#00ADC5] rounded-lg" placeholder="Position Name">
                                    </td>
                                    <td class="px-4 py-4">
                                        <input type="text" name="current_holder[]" required x-model="row.current_holder" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:border-[#00ADC5] rounded-lg" placeholder="Current Employee">
                                    </td>
                                    <td class="px-4 py-4">
                                        <input type="text" name="candidates[]" required x-model="row.candidates" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:border-[#00ADC5] rounded-lg" placeholder="Succession Candidates">
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="timeline_progress[]" x-model="row.timeline_progress" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm" placeholder="Timeline details..."></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="competency_progress[]" x-model="row.competency_progress" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm" placeholder="Skills progress..."></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="development_progress[]" x-model="row.development_progress" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm" placeholder="Training/Mentorship..."></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="kpi_metrics[]" x-model="row.kpi_metrics" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm" placeholder="KPI scores..."></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="monitoring_progress[]" x-model="row.monitoring_progress" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm" placeholder="Notes..."></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <select name="readiness_rating[]" required x-model="row.readiness_rating" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-[10px] font-black uppercase text-slate-700 outline-none focus:border-[#00ADC5] rounded-lg">
