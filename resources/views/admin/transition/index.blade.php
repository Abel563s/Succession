        ...
        <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
                <x-admin-module-help
                    ...
                    title="Transition Plan Guide"
                    description="Support a confident handover by monitoring progress, evaluating readiness, and documenting approvals before the nominee assumes the role."
                    :sections="[
                        ['icon' => 'refresh-cw', 'title' => 'Purpose', 'content' => 'Facilitate the successful transfer of responsibilities from the current role holder to the nominee.'],
                        ['icon' => 'list', 'title' => 'Instructions', 'content' => [
                            'Monitor progress against development activities and IDP objectives.',
                            'Evaluate readiness based on performance, competency improvement, training completion, and demonstrated capability.',
                            'Initiate transition when readiness requirements are met.',
                        ]],
                        ['icon' => 'user-check', 'title' => 'Required Approvals', 'content' => [
                            'Line Manager',
                            'HR Representative',
                            'Deputy Chief Executive Officer (DCEO)',
                        ]],
                        ['icon' => 'award', 'title' => 'Outcome', 'content' => 'Upon successful completion and approval, the nominee assumes the critical role and full responsibilities of the position.'],
                    ]"
                />
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm inline-block">
                <form action="{{ route('admin.transition.index') }}" method="GET" class="flex flex-wrap items-end justify-center gap-4">
                    ...
                    <div class="w-40 text-left">
                        ...
                        <select name="status" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#111111] outline-none text-xs font-bold text-slate-700 appearance-none">
                                    ...
                                    <div class="flex flex-col gap-1 max-w-xs">
                                        @foreach($record->items as $item)
                                            <div class="flex items-center justify-between gap-4 text-xs font-medium text-slate-700 bg-slate-50 border border-slate-100 rounded-lg p-2">
                                                <div>
                                                    <span class="font-black text-[#111111]">{{ $item->critical_role }}</span>
                                                    <div class="text-[10px] text-slate-400 mt-0.5">{{ $item->current_holder }} &rarr; <span class="font-bold text-[#D4AF37]">{{ $item->successor }}</span></div>
                                                </div>
