                        ...
                        @if($training->signature_path)
                            <div class="md:col-span-2">
                                ...
                                <div class="bg-slate-50 p-3 rounded-2xl border border-slate-100 inline-block">
                                    <img src="{{ asset('storage/' . $training->signature_path) }}" class="h-20 object-contain" alt="Legacy Signature">
                                </div>
                            </div>
                        @else
                            <div>
                                ...
                                <div class="bg-slate-50 p-3 rounded-2xl border border-slate-100 inline-block">
                                    @if($training->manager_signature)
                                        <img src="{{ asset('storage/' . $training->manager_signature) }}" class="h-20 object-contain" alt="Manager Signature">
                                    @else
                            ...
                            <div>
                                ...
                                <div class="bg-slate-50 p-3 rounded-2xl border border-slate-100 inline-block">
                                    @if($training->candidate_signature)
                                        <img src="{{ asset('storage/' . $training->candidate_signature) }}" class="h-20 object-contain" alt="Candidate Signature">
                                    @else
                                        <div class="h-20 w-40 flex items-center justify-center border-2 border-dashed border-slate-200 rounded-xl">
                                            <span class="text-xs font-bold text-slate-400">N/A</span>
                                        </div>
        ...
        @if(auth()->user()->isAdmin())
            <div class="bg-rose-50 rounded-[2rem] border border-rose-100 p-8 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-rose-600/70 text-sm font-medium">Permanently remove this training record from the system.</p>
                </div>
                <form action="{{ route('admin.training.destroy', $training) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?');">
                    @csrf
                    @method('DELETE')
