    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4 pb-4 border-b border-slate-100">
            <div class="flex flex-col">
                <h1 class="text-2xl font-black text-slate-900 tracking-tight">Training Programs</h1>
                <p class="text-xs text-slate-400 font-medium">Monitoring skill acquisition and goal-based training performance.</p>
            </div>
            ...
            <div class="hidden lg:flex items-center justify-center">
                <div class="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-xl hover:border-[#111111]/20 transition-all duration-500 group/badge">
                    <div class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-[#111111] transition-colors group-hover/badge:bg-[#111111] group-hover/badge:text-white shadow-inner">
                        <i data-lucide="graduation-cap" class="w-5 h-5 transition-transform group-hover/badge:scale-110"></i>
                    </div>
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest group-hover/badge:text-slate-600 transition-colors">Total Trainings</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
                <x-admin-module-help
                    ...
                    description="Capture training needs that close competency gaps and build the successor's readiness for future role requirements."
                    :sections="[
                        ['icon' => 'graduation-cap', 'title' => 'Purpose', 'content' => 'Address competency gaps through formal learning and development interventions.'],
                        ['icon' => 'list', 'title' => 'Instructions', 'content' => [
                            'Identify required training programs based on assessment results.',
                            'Include technical training, leadership training, and behavioral development programs.',
                            'Ensure all recommendations directly support succession readiness.',
                        ]],
                        ['icon' => 'alert-circle', 'title' => 'Key Note', 'content' => 'Training recommendations should align with identified competency gaps and future role requirements.' ],
                    ]"
                />
                ...
                <a href="{{ route('admin.training.create') }}" 
                   class="bg-gradient-to-r from-[#111111] to-[#00333B] hover:to-[#111111] text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all duration-300 shadow-lg shadow-[#111111]/20 hover:shadow-[#111111]/30 hover:scale-[1.02] active:scale-95 group/btn relative overflow-hidden">
                    ...
                    <span class="relative z-10">Add New Training</span>
                </a>
            </div>
        </div>
        ...
        <div class="flex justify-center">
            <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm inline-block">
                <form action="{{ route('admin.training.index') }}" method="GET" class="flex flex-wrap items-end justify-center gap-4">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gradient-to-r from-[#111111] to-[#00333B] text-white text-[10px] font-black uppercase tracking-widest">
                            ...
                            <th class="px-6 py-4 border-b border-white/5">Candidate</th>
                            <th class="px-6 py-4 border-b border-white/5">Department / Manager</th>
                            <th class="px-6 py-4 border-b border-white/5">Goal 1 Details</th>
                            <th class="px-6 py-4 border-b border-white/5 text-center">Scores (1-3)</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group text-sm">
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="font-bold text-slate-900 leading-tight">{{ $record->candidate_name }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    <div class="text-[10px] font-black uppercase text-slate-400 tracking-widest">{{ $record->department }}</div>
                                    <div class="text-xs font-bold text-slate-600 mt-0.5">{{ $record->line_manager }}</div>
                                </td>
                                <td class="px-6 py-5">
                                    <div class="max-w-xs">
                                        <div class="text-xs font-bold text-slate-800 leading-tight line-clamp-1">{{ $record->goal_1 }}</div>
                                        <div class="text-[10px] font-black text-[#111111] uppercase tracking-wider mt-1">{{ $record->skill_area_1 }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                    <div class="flex items-center justify-center gap-2">
                                        <span class="w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center text-xs font-black text-slate-900">{{ $record->score_1 ?: '-' }}</span>
                                        <span class="w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center text-xs font-black text-slate-900">{{ $record->score_2 ?: '-' }}</span>
                                        <span class="w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center text-xs font-black text-slate-900">{{ $record->score_3 ?: '-' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        @if(auth()->user()->isAdmin())
                                            <form action="{{ route('admin.training.destroy', $record) }}" method="POST" 
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-20 text-center text-slate-400 font-medium italic">No training records found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
