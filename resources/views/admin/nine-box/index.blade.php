    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
            <div class="flex flex-col">
                <h1 class="text-2xl font-black text-slate-900 tracking-tight">9-Box Grid Evaluations</h1>
                <p class="text-xs text-slate-400 font-medium">Mapped matrix of talent potential vs performance.</p>
            </div>
            ...
            <div class="hidden lg:flex items-center justify-center">
                <div class="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-xl hover:border-[#00515F]/20 transition-all duration-500 group/badge">
                    <div class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-[#00515F] transition-colors group-hover/badge:bg-[#00515F] group-hover/badge:text-white shadow-inner">
                        <i data-lucide="grid" class="w-5 h-5 transition-transform group-hover/badge:scale-110"></i>
                    </div>
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest group-hover/badge:text-slate-600 transition-colors">Total Evaluations</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex md:justify-end">
                <a href="{{ route('admin.nine-box.create') }}" 
                   class="bg-gradient-to-r from-[#00515F] to-[#00333B] hover:to-[#00515F] text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all duration-300 shadow-lg shadow-[#00515F]/20 hover:shadow-[#00515F]/30 hover:scale-[1.02] active:scale-95 group/btn relative overflow-hidden">
                    ...
                    <span class="relative z-10">New Evaluation</span>
                </a>
            </div>
        </div>
        ...
        <div class="flex justify-center">
            <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm inline-block">
                <form action="{{ route('admin.nine-box.index') }}" method="GET" class="flex flex-wrap items-end justify-center gap-4">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($records as $record)
                            <tr class="hover:bg-slate-50/50 transition-colors group text-sm">
                                ...
                                <td class="px-6 py-5">
                                    <span class="inline-flex px-3 py-1 rounded-lg bg-[#f0fbfd] text-[#00ADC5] text-[10px] font-black uppercase border border-[#00ADC5]/10 tracking-wider">
                                        {{ $record->grid_position }}
                                    </span>
                                </td>
                                <td class="px-6 py-5">
                                    <div class="flex flex-col">
                                        <span class="text-xs font-bold text-slate-700">P: {{ $record->potential_level }}</span>
                                        <span class="text-[10px] text-slate-400 font-medium">Perf: {{ $record->performance_level }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        @if(auth()->user()->isAdmin())
                                            <a href="{{ route('admin.nine-box.edit', $record) }}" 
                                            ...
                                            <form action="{{ route('admin.nine-box.destroy', $record) }}" method="POST" 
