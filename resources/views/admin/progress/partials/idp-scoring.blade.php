            ...
            <table class="w-full border-collapse text-sm">
                ...
                <tbody class="divide-y divide-slate-50">
                    @foreach($progress->idpObjectives->sortBy('row_number') as $objective)
                        <tr>
                            <td class="px-4 py-4 text-center font-black text-[#00ADC5]">{{ $objective->row_number }}</td>
                            ...
                            <td class="px-4 py-4">
                                <input type="text" name="idp_objectives[{{ $objective->id }}][score]"
                                       ...
                                       class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-center text-xs font-black text-slate-900 focus:border-[#00ADC5] outline-none"
                                       placeholder="Score">
                            </td>
                        </tr>
                    @endforeach
