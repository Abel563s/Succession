        ...
        <form action="{{ route('admin.progress.update', $progress) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8 space-y-4">
                    <div class="space-y-2">
                        ...
                        <textarea name="performance_summary" rows="5" class="w-full px-6 py-4 bg-slate-50 border border-slate-200 focus:border-[#D4AF37] outline-none transition-all text-sm font-medium resize-none shadow-sm" placeholder="Summarize overall development progress...">{{ old('performance_summary', $progress->performance_summary) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden" 
                ...
                <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
                    ...
                    <button type="button" @click="rows.push({review_date: '', achievements: '', challenges: '', next_steps: ''})" class="text-xs font-black text-[#D4AF37] uppercase tracking-widest hover:text-[#008d9e] flex items-center gap-2 bg-[#FFF8E7] px-4 py-2 rounded-xl transition-all border border-[#D4AF37]/10">
                            ...
                            <template x-for="(row, index) in rows" :key="index">
                                <tr class="border-b border-slate-50 hover:bg-slate-50/30 transition-colors">
                                    <td class="px-8 py-4 align-top">
                                        <input type="date" name="review_date[]" x-model="row.review_date" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:border-[#D4AF37] rounded-lg">
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="achievements[]" x-model="row.achievements" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#D4AF37] rounded-lg resize-none shadow-sm"></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="challenges[]" x-model="row.challenges" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#D4AF37] rounded-lg resize-none shadow-sm"></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="next_steps[]" x-model="row.next_steps" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#D4AF37] rounded-lg resize-none shadow-sm"></textarea>
                                    </td>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <textarea name="new_competencies" rows="4" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#D4AF37] outline-none transition-all text-sm font-medium resize-none shadow-sm">{{ old('new_competencies', $progress->new_competencies) }}</textarea>
                    </div>
                    <div class="space-y-2">
                        <label class="text-[11px] font-black uppercase tracking-widest text-slate-400 ml-1">Gaps Identified</label>
                        <textarea name="gaps_identified" rows="4" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#D4AF37] outline-none transition-all text-sm font-medium resize-none shadow-sm">{{ old('gaps_identified', $progress->gaps_identified) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8 space-y-4">
                    <div class="space-y-2">
                        ...
                        <textarea name="updated_action_plan" rows="5" class="w-full px-6 py-4 bg-slate-50 border border-slate-200 focus:border-[#D4AF37] outline-none transition-all text-sm font-medium resize-none shadow-sm">{{ old('updated_action_plan', $progress->updated_action_plan) }}</textarea>
                    </div>
                </div>
            </div>
