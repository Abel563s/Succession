    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Progress Review</h1>
                    <p class="text-slate-500 font-medium text-sm">Update strategic performance tracking for {{ $progress->candidate_name }}.</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin.progress.update', $progress) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="candidate_name" required value="{{ old('candidate_name', $progress->candidate_name) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="department" required
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept->name }}" {{ old('department', $progress->department) == $dept->name ? 'selected' : '' }}>{{ $dept->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="space-y-2">
                        <label class="text-[11px] font-black uppercase tracking-widest text-slate-400 ml-1">Line Manager</label>
                        <input type="text" name="line_manager" required value="{{ old('line_manager', $progress->line_manager) }}"
                               class="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#00ADC5] outline-none transition-all font-bold text-slate-700"
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/30">
                    <h2 class="text-lg font-black text-slate-800 flex items-center gap-2 uppercase tracking-tight">
                        <i data-lucide="clipboard-list" class="w-5 h-5 text-[#00ADC5]"></i>
                        Performance Summary
                    </h2>
                </div>
                <div class="p-8 space-y-4">
                    <div class="space-y-2">
                        ...
                        <textarea name="performance_summary" rows="5" class="w-full px-6 py-4 bg-slate-50 border border-slate-200 focus:border-[#00ADC5] outline-none transition-all text-sm font-medium resize-none shadow-sm" placeholder="Summarize overall development progress...">{{ old('performance_summary', $progress->performance_summary) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden" 
                 x-data="{ rows: {{ json_encode($progress->trackingItems->map(fn($item) => [
                ...
                <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/30 flex items-center justify-between">
                    ...
                    <button type="button" @click="rows.push({review_date: '', achievements: '', challenges: '', next_steps: ''})" class="text-xs font-black text-[#00ADC5] uppercase tracking-widest hover:text-[#008d9e] flex items-center gap-2 bg-[#f0fbfd] px-4 py-2 rounded-xl transition-all border border-[#00ADC5]/10">
                            ...
                            <template x-for="(row, index) in rows" :key="index">
                                <tr class="border-b border-slate-50 hover:bg-slate-50/30 transition-colors">
                                    <td class="px-8 py-4 align-top">
                                        <input type="date" name="review_date[]" x-model="row.review_date" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-bold text-slate-700 outline-none focus:border-[#00ADC5] rounded-lg">
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="achievements[]" x-model="row.achievements" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm"></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="challenges[]" x-model="row.challenges" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm"></textarea>
                                    </td>
                                    <td class="px-4 py-4">
                                        <textarea name="next_steps[]" x-model="row.next_steps" rows="2" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 text-xs font-medium text-slate-600 outline-none focus:border-[#00ADC5] rounded-lg resize-none shadow-sm"></textarea>
                                    </td>
            ...
            @include('admin.progress.partials.idp-scoring', ['progress' => $progress])

            <!-- Section 4: Skill Development -->
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/30">
                    <h2 class="text-lg font-black text-slate-800 flex items-center gap-2 uppercase tracking-tight">
                        <i data-lucide="graduation-cap" class="w-5 h-5 text-[#00ADC5]"></i>
                        Skill Development
                    </h2>
                </div>
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <textarea name="new_competencies" rows="4" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#00ADC5] outline-none transition-all text-sm font-medium resize-none shadow-sm">{{ old('new_competencies', $progress->new_competencies) }}</textarea>
                    </div>
                    <div class="space-y-2">
                        <label class="text-[11px] font-black uppercase tracking-widest text-slate-400 ml-1">Gaps Identified</label>
                        <textarea name="gaps_identified" rows="4" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#00ADC5] outline-none transition-all text-sm font-medium resize-none shadow-sm">{{ old('gaps_identified', $progress->gaps_identified) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-8 space-y-4">
                    <div class="space-y-2">
                        ...
                        <textarea name="updated_action_plan" rows="5" class="w-full px-6 py-4 bg-slate-50 border border-slate-200 focus:border-[#00ADC5] outline-none transition-all text-sm font-medium resize-none shadow-sm">{{ old('updated_action_plan', $progress->updated_action_plan) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="p-8 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div class="flex-1 space-y-4">
                        ...
                        <div x-data="{ preview: '{{ $progress->signature_path ? \App\Support\StorageUrl::public($progress->signature_path) : null }}' }" class="space-y-4">
                    ...
                    <div class="flex-shrink-0 space-y-3">
                        ...
                        <select name="status" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 focus:border-[#00ADC5] outline-none transition-all font-bold text-slate-700 rounded-xl">
                            <option value="draft" {{ old('status', $progress->status) == 'draft' ? 'selected' : '' }}>Draft Review</option>
                            <option value="completed" {{ old('status', $progress->status) == 'completed' ? 'selected' : '' }}>Completed Review</option>
                        </select>
                    </div>
                </div>
            </div>
