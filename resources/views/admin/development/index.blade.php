    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
            <div class="flex flex-col">
                <h1 class="text-2xl font-black text-slate-900 tracking-tight">Development Plans</h1>
                <p class="text-xs text-slate-400 font-medium">Monitoring Individual Development Plans (IDP) and talent growth.</p>
            </div>
            ...
            <div class="hidden lg:flex items-center justify-center">
                <div class="flex items-center gap-3 px-4 py-2 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-xl hover:border-[#111111]/20 transition-all duration-500 group/badge">
                    <div class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-[#111111] transition-colors group-hover/badge:bg-[#111111] group-hover/badge:text-white shadow-inner">
                        <i data-lucide="target" class="w-5 h-5 transition-transform group-hover/badge:scale-110"></i>
                    </div>
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest group-hover/badge:text-slate-600 transition-colors">Total IDPs</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex md:justify-end">
                <a href="{{ route('admin.development.create') }}" 
                   class="bg-gradient-to-r from-[#111111] to-[#00333B] hover:to-[#111111] text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all duration-300 shadow-lg shadow-[#111111]/20 hover:shadow-[#111111]/30 hover:scale-[1.02] active:scale-95 group/btn relative overflow-hidden">
                    ...
                    <span class="relative z-10">New IDP Form</span>
                </a>
            </div>
        </div>
        ...
        <div class="flex justify-center">
            <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm inline-block">
                <form action="{{ route('admin.development.index') }}" method="GET" class="flex flex-wrap items-end justify-center gap-4">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gradient-to-r from-[#111111] to-[#00333B] text-white text-[10px] font-black uppercase tracking-widest">
                            ...
                            <th class="px-6 py-4 border-b border-white/5">Employee & Manager</th>
                            <th class="px-6 py-4 border-b border-white/5">Department</th>
                            <th class="px-6 py-4 border-b border-white/5">Objectives</th>
                            <th class="px-6 py-4 border-b border-white/5 text-center">Avg Score</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($records as $record)
                            @php
                                $avgScore = $record->objectives->avg(function($obj) {
                                    return is_numeric($obj->score) ? $obj->score : null;
                                }) ?: 0;
                            @endphp
                            <tr class="hover:bg-slate-50/50 transition-colors group text-sm">
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl bg-[#FFF8E7] border border-[#D4AF37]/10 flex items-center justify-center font-black text-[#D4AF37]">
                                            {{ substr($record->employee_name, 0, 1) }}
                                        </div>
                                        <div>
                                            <div class="font-bold text-slate-900 leading-tight">{{ $record->employee_name }}</div>
                                            <div class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">Manager: {{ $record->line_manager }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex items-center gap-2 text-slate-500 text-xs font-medium">
                                        <i data-lucide="target" class="w-3.5 h-3.5 text-[#D4AF37]"></i>
                                        <span class="font-bold text-slate-700">{{ $record->objectives->count() }} active items</span>
                                    </div>
                                </td>
                                <td class="px-6 py-5 text-center">
                                    <span class="text-sm font-black text-slate-900">{{ number_format($avgScore, 1) }}</span>
                                </td>
                                <td class="px-6 py-5 text-center">
                                ...
                                <td class="px-6 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('admin.development.show', $record) }}" 
                                        ...
                                        @if(auth()->user()->isAdmin() || (auth()->user()->isManager() && ($record->approval_status === 'Pending' || (($record->created_by ?? $record->user_id) == auth()->id() && (!$record->approval_status || $record->approval_status === 'Pending')))))
                                            <a href="{{ route('admin.development.edit', $record) }}" 
                                        ...
                                        @if(auth()->user()->isAdmin())
                                            <form action="{{ route('admin.development.destroy', $record) }}" method="POST" 
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-20 text-center text-slate-400 font-medium italic">No development plans found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
