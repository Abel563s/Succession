    ...
    <div class="max-w-[1400px] mx-auto space-y-8 font-inter">
        ...
        <form action="{{ route('admin.development.update', $development) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="employee_name" required value="{{ old('employee_name', $development->employee_name) }}"
                               class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-[#D4AF37] focus:ring-4 focus:ring-[#D4AF37]/10 outline-none transition-all font-bold text-slate-700"
                            ...
                            @for($i = 0; $i < max(5, $development->objectives->count()); $i++)
                                ...
                                <tr class="group hover:bg-slate-50/50 transition-all">
                                    <td class="px-4 py-4 text-center bg-slate-50/50 group-hover:bg-[#FFF8E7] transition-all">
                                        <span class="text-xs font-black text-[#D4AF37]">{{ $i + 1 }}</span>
                                    </td>
                                    <td class="px-2 py-2">
            ...
            <div class="flex flex-col items-center pb-20">
                <button type="submit" 
                        class="px-16 py-5 bg-slate-900 text-white rounded-[2rem] font-black uppercase tracking-[0.3em] text-[11px] shadow-2xl shadow-slate-900/40 hover:bg-[#D4AF37] transition-all duration-500 active:scale-[0.98]">
                    UPDATE IDP REPORT
                </button>
