...
<div {{ $attributes->merge(['class' => 'space-y-4']) }} x-data="{ mode: '{{ $canUseSaved ? 'saved' : 'upload' }}', preview: null }">
    ...
    @if($canUseSaved)
        ...
        <div x-show="mode === 'saved'" x-cloak class="p-4 border border-slate-100 rounded-2xl bg-slate-50 flex items-center justify-center">
            <img src="{{ $savedUrl }}" class="max-h-28 rounded-xl shadow-sm border border-white" alt="Saved signature">
        </div>
    @endif
    ...
    <div x-show="{{ $canUseSaved ? "mode === 'upload'" : 'true' }}" x-cloak>
        ...
        <div x-show="preview" x-transition class="mt-4 p-4 border border-dashed border-slate-200 rounded-2xl bg-slate-50 flex items-center justify-center">
            <img :src="preview" class="max-h-28 rounded-xl shadow-sm" alt="Signature preview">
        </div>
    </div>
</div>
